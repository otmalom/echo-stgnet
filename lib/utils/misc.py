...
def load_model(ckpt_obj, model, optimizer=None, warning=True):
    if isinstance(ckpt_obj, dict):
        checkpoint = ckpt_obj
    else:
        print("=> loading checkpoint '{}'".format(ckpt_obj))
...
def print_speed(i, i_time, n):
    ...
    remaining_second = math.floor(remaining_time - remaining_day * 86400 - remaining_hour * 3600 - remaining_min*60)
    
    info_str = 'Remaining time: %d:%02d:%02d:%02d (D:H:M:S)' % (
        remaining_day, remaining_hour, remaining_min, remaining_second)
    return info_str
