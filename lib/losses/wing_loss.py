...
from lib.losses import functional as F
import numpy as np
import scipy.sparse as sp


def gen_laplacian_matrix(adj_dir, num_points):
    """
...
class LaplacianLoss(_Loss):
    def __init__(self, loss, adj_dir, num_points, reduction="mean"):
        super(LaplacianLoss, self).__init__(reduction=reduction)
        self.loss = loss
        self.num_points = num_points
        self.laplacian_matrix = gen_laplacian_matrix(adj_dir,num_points).cuda() # 106x106
